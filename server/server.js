...
const CMC_API_KEY = 'e5e27344-c914-4d10-861b-ed8f6393eaf5';
...
app.get('/api/cryptocurrency/listings/latest', async (req, res) => {
  try {
    ...
    console.error('Error fetching cryptocurrency listings:', error.response?.data || error.message);
    res.status(500).json({
...
app.get('/api/cryptocurrency/quotes/latest', async (req, res) => {
  try {
    ...
    console.error('Error fetching cryptocurrency quotes:', error.response?.data || error.message);
    res.status(500).json({
...
app.get('/api/cryptocurrency/info', async (req, res) => {
  try {
    ...
    console.error('Error fetching cryptocurrency info:', error.response?.data || error.message);
    res.status(500).json({
